...
function save() {
  ...
  formError.value = false; // Reset error state
  form.value.validate().then((success: { valid: boolean }) => {
    if (!success.valid) {
      formError.value = true; // Show error notification
    } else {
      formError.value = false;
      if (isEditing.value) {
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="container mx-auto p-4">
      <div class="flex justify-between items-center mb-4">
        ...
        <v-btn class="me-2" prepend-icon="mdi-plus" rounded="lg" text="Crear Comensal" border @click="add"></v-btn>
      </div>
      ...
      <v-form ref="form" @submit.prevent="save">
        <v-card :subtitle="`${isEditing ? 'Cambia' : 'Añade'} la información un comensal`"
          ...
          <template v-slot:text>
            ...
            <v-text-field v-model="record.email" :rules="rules.email" label="Correo Electrónico"></v-text-field>
            <v-text-field v-model="record.telephone" :rules="rules.telephone" label="Teléfono"></v-text-field>
